...
def game(dt, round, seq):
    ...
    for event in seq:
        if event == 0:
            buttons[0] = Fore.RED + 'W' + Style.RESET_ALL
        if event == 1:
            buttons[1] = Fore.GREEN + 'A' + Style.RESET_ALL
        if event == 2:
            buttons[2] = Fore.YELLOW + 'D' + Style.RESET_ALL
        if event == 3:
            buttons[3] = Fore.BLUE + 'S' + Style.RESET_ALL
        disp_butt(buttons, round)
        time.sleep(dt*2)
        buttons = ['w', 'a', 'd', 's']
        disp_butt(buttons, round)
